...
from haystack.components.embedders import SentenceTransformersTextEmbedder, SentenceTransformersDocumentEmbedder, OpenAITextEmbedder, OpenAIDocumentEmbedder
...
class RetrieverPipeline:
    ...
    def _initialize_pipeline(self):
        ...
        document_embedder = SentenceTransformersDocumentEmbedder(model=self.model, device=device)
        ...
        self.query_pipeline.add_component("text_embedder", SentenceTransformersTextEmbedder(model=self.model, prefix=self.instruction, device=device))
...
class OpenAIRetrieverPipeline:
    def __init__(self, labels_file: str, model: str = "text-embedding-3-large", distance_function: str = "cosine", instruction: str = ""):
        self.labels_file = labels_file
        self.model = model
        self.distance_function = distance_function
        self.instruction = instruction
    ...
    def _initialize_pipeline(self):
        df_labels = self._load_labels(self.labels_file)
        self.document_store = ChromaDocumentStore(distance_function=self.distance_function)
        api_key = Secret.from_token("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")
        # Initialize the document embedder
        document_embedder = OpenAIDocumentEmbedder(api_key=api_key, model=self.model)
        # document_embedder.warm_up()
        ...
        # Create and connect components in the pipeline
        self.query_pipeline = Pipeline()
        self.query_pipeline.add_component("text_embedder", OpenAITextEmbedder(api_key=api_key, model=self.model))
        self.query_pipeline.add_component("retriever", ChromaEmbeddingRetriever(document_store=self.document_store))
        self.query_pipeline.connect("text_embedder.embedding", "retriever.query_embedding")
