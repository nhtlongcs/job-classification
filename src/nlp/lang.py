...
def translate_process_data(df_dataset, verbose=False):
    ...
    df_dataset["final_description"] = df_dataset.progress_apply(
        lambda x: (
            x["title"]
            if any(
                keyword in x["description"]
                for keyword in unwanted_keywords
            )
            or len(x["description"]) < 50
            else x["title"] + " " + x["description"]
        ),
        axis=1,
    )

    df_dataset["final_description_en"] = df_dataset.progress_apply(
        lambda x: (
            x["translated_title"]
            if any(
        ...
        if pd.notna(x["translated_title"]) and pd.notna(x["translated_description"])
        else (x["translated_title"] if pd.notna(x["translated_title"]) 
        else (x["translated_description"] if pd.notna(x["translated_description"]) 
        else x["final_description"])),
        axis=1,
    )
...
def fuzzy_matching_df(merged_df, enriched_df_path, en_dataset_path):
    ...
    merged_df["Final Prediction"] = merged_df["Final Prediction"].astype(int)
    return (
