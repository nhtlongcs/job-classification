...
def merge_predictions(codes):
    ...
    for code, score in codes:
        code_dict[
            str(code)
        ] += score  # Merge scores if same code appears multiple times
...
def parse_args():
    parser = argparse.ArgumentParser(description="Merge ISCO code predictions from multiple files.")
    parser.add_argument(
        "--ensemble_files",
        nargs='+',
        required=True,
        help="List of CSV files containing predictions to be merged."
    )
    parser.add_argument(
        "--compare_file",
        required=True,
        help="CSV file to compare the final predictions against."
    )
    return parser.parse_args()

args = parse_args()
ensemble_files = args.ensemble_files
...
def main():
    ...
    for i, df in enumerate(dfs):
        ...
        dfs[i][prediction_col] = (
            dfs[i][prediction_col]
            .astype(int)
            .astype(str)
            .apply(lambda x: x.zfill(4))
        )
        assert len(uids.intersection(set(df["id"].unique()))) == len(
            uids
        ), f"Dataframe {i} should have the same ids as the first dataframe"
    ...
    for uid in tqdm(uids):
        ...
        for i, df in enumerate(dfs):
            ...
            if code == "0000":
                ...
                score = (
                    df.loc[uid]["Confidence"]
                    if df.loc[uid]["Confidence"]
                    else 0.7
                )
            score = score * factors[i]
            codes.append((code, score))
            final_df[Path(ensemble_files[i]).stem].append(
                df.loc[uid][prediction_col]
            )
    ...
    for uid in uids:
        if (
            final_df.loc[uid][prediction_col]
            != compare_df.loc[uid][prediction_col]
        ):
            print(
                f"{uid}: {compare_df.loc[uid][prediction_col]} -> {final_df.loc[uid][prediction_col]}"
            )
            count += 1
...
# example usage
# python merge.py --ensemble_files ensemble1.csv ensemble2.csv ensemble3.csv --compare_file latest_predictions.csv