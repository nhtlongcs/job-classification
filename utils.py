...
def parse_final_prediction(response):
    ...
    alternative_prediction_pattern = r"(?:\*{1,2})?Alternative ISCO Codes \(unit\):\s*([\d, ]+)"
    ...
    for part in tqdm.tqdm(parts):
        ...
        if matches_job_id:
            ...
            final_prediction_section = part.split("Final Prediction:")[-1] if "Final Prediction:" in part else part

            matches_prediction = re.findall(prediction_pattern, final_prediction_section)
            matches_alternative_prediction = re.findall(alternative_prediction_pattern, final_prediction_section)
            matches_confidence = re.findall(confidence_pattern, final_prediction_section)
            ...
            if matches_alternative_prediction:
                alt_codes = matches_alternative_prediction[0].strip().split(', ')
                alt_codes = ['0' + code if len(code) == 3 else code for code in alt_codes]
...
def drop_duplicates_prioritize_non_null(df: pd.DataFrame, id_col='id', isco_code_col='ISCO Code') -> pd.DataFrame:
    """
    ...
    df_sorted = df.sort_values(by=[id_col, 'is_non_null'], ascending=[True, False])

    print(f"Number of duplicated rows: {df_sorted.duplicated(subset=id_col).sum()}")
    print(f"Number of duplicated rows with non-null ISCO Code: {df_sorted[df_sorted['is_non_null']][id_col].duplicated().sum()}")
    ...
    df_deduplicated = df_deduplicated.drop(columns=['is_non_null']).reset_index(drop=True)

    return df_deduplicated


def drop_duplicates_prioritize_non_null_with_confidence(df: pd.DataFrame, id_col='id', isco_code_col='ISCO Code', confidence_col='Confidence') -> pd.DataFrame:
    """
    ...
    df_sorted = df.sort_values(by=[id_col, 'is_non_null', confidence_col], ascending=[True, False, False])
